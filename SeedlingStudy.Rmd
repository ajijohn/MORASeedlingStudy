...
```{r, echo=FALSE}
photosynq <- read_csv('~/Desktop/photosynq.csv')
summary(photosynq)
```
...
```{r, echo=FALSE}
photosynq %>% ggplot() + geom_bar(aes(x=Species,y=Phi2),stat = "identity") + facet_grid(.~Stand) +
...
```{r, echo=FALSE}
photosynq %>% ggplot() + geom_bar(aes(x=Species,y='PAR'),stat = "identity") + facet_grid(.~Stand) +
